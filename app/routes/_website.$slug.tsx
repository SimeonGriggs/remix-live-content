...
export const loader = async ({params, request}: LoaderFunctionArgs) => {
  ...
  let lastLiveEventId = requestUrl.searchParams.get('lastLiveEventId')

  const initial = await client
    .fetch(query, params, {filterResponse: false, lastLiveEventId})
    .then((res) => {
      syncTags = res.syncTags
      return res?.result ? {data: recordZ.parse(res.result)} : null
    })
...
export default function RecordPage() {
  const {data, syncTags} = useLoaderData<typeof loader>()
